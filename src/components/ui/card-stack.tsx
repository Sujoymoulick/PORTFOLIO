...
export function CardStack<T extends CardStackItem>({
  ...
  const currentCardWidth = isMobile ? Math.min(340, width - 40) : cardWidth;
  const currentCardHeight = isMobile
    ? Math.min(220, Math.round(currentCardWidth / 1.625))
    : cardHeight;
  ...
  const cardSpacing = isMobile
    ? Math.max(10, Math.round(currentCardWidth * 0.16))
    : Math.max(10, Math.round(currentCardWidth * (1 - overlap)));
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{ height: Math.max(isMobile ? 290 : 380, currentCardHeight + 80) }}
        tabIndex={0}
        onKeyDown={onKeyDown}
      >
...
function DefaultFanCard({ item }: { item: CardStackItem; active: boolean }) {
  return (
    <div className="relative h-full w-full">
      ...
      {/* content */}
      <div className="relative z-10 flex h-full flex-col justify-end p-4 md:p-8">
        <div className="flex items-center gap-2 mb-2">
           <span className="text-[8px] md:text-[10px] font-bold uppercase tracking-widest px-2 py-0.5 rounded bg-white/10 backdrop-blur-md border border-white/20 text-white/90">
             {item.tag || "Certification"}
           </span>
        </div>
        <div className="truncate text-lg md:text-2xl font-display font-bold text-white mb-1">
          {item.title}
        </div>
        {item.description ? (
          <div className="mt-1 line-clamp-2 text-xs md:text-sm text-white/60 font-medium font-sans">
            {item.description}
          </div>
        ) : null}
      </div>
